...
def plot_slopeWalking_gamma_beta_pitch_displacement(data_file_dic,start_point=90,end_point=1200,freq=60.0,inclinations=['-0.2618','0.0','0.2618']):
    ...
    for name, inclination in data_date_inclination: #name is a inclination names
        ...
        for idx in inclination.index:
            ...
            # 2)  data process
            print(folder_name)
            gamma[name].append(COG_distribution(grf_data[start_point:end_point,:]))

            gait_diagram_data_temp, beta_temp=gait(grf_data[start_point:end_point,:])
            gait_diagram_data[name].append(gait_diagram_data_temp); beta[name].append(beta_temp)

            pitch[name].append(pose_data[start_point:end_point,1])

            displacement[name].append(pose_data[start_point:end_point,3]/np.cos(float(name)))
            jmc[name].append(command_data[start_point:end_point,:])
...
def slopeWalking_gamma_beta_pitch_displacement_statistic():
    # 1) read data
    start_point= 900#2100
    freq=60.0 # 60Hz,
    end_point=1200

    #1) load data from file
    data_file_dic= "/media/suntao/DATA/Research/P3_workspace/Figures/slopeWalking/data/coupledCPGswithCOGReflexes/"
    data_file = data_file_dic +"slopeWalking.log"
    data_date = pd.read_csv(data_file, sep='\t',header=None, names=['file_name','inclination'], skip_blank_lines=True,dtype=str)
...
def slopeWalking_getTouchData(data_file_dic,start_point=900,end_point=1200,freq=60):
    ...
    for name, inclination in data_date_inclination: #name is a inclination names
        ...
        for idx in inclination.index: # how many time experiments one inclination
            ...
            # 2)  data process
            print(folder_name)
            Te_temp=touch_difference(cpg_data[start_point:end_point,[0,2,4,6]], grf_data[start_point:end_point,:])
            ...
            Te[name].append([Te_temp,command_data[start_point:end_point,:]])
...
if __name__=="__main__":
    ...
    #data_file_dic= "/media/suntao/DATA/Research/P3_workspace/Figures/slopeWalking/data/coupledCPGswithCOGReflexes/"
    #plot_slopeWalking_gamma_beta_pitch_displacement(data_file_dic,start_point=500,end_point=1500,freq=60.0,inclinations=['0'])
    ...
    plot_adaptiveOffset(data_file_dic,start_point=300,end_point=660,freq=60.0,inclinations=['0'])
